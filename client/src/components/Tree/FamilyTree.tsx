...
// import EditPersonModal from '../EditPersonModal'; // Already imported above
...
export default function FamilyTree() {
    ...
    const { currentFamilyTree } = useAuth();
        ...
        <div className="w-full h-screen bg-slate-950"> {/* Dark background */}
            <ReactFlowProvider>
                ...
                >
                    ...
                    />
                    {isPending && (
                        <div className="absolute top-4 left-4 z-50 bg-blue-600 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg animate-pulse">
                            Wait...
                        </div>
                    )}

                    {/* Empty States Overlay */}
                    {(!currentFamilyTree || (people && people.length === 0)) && (
                        <div className="absolute inset-0 z-10 bg-slate-950/80 backdrop-blur-sm p-4 md:p-20 flex items-center justify-center">
                            {!currentFamilyTree ? (
                                <EmptyState
                                    icon={TreeDeciduous}
                                    title="No Family Tree Selected"
                                    description="Please select an existing family tree or create a new one to get started."
                                />
                            ) : (
                                <EmptyState
                                    icon={Users}
                                    title={`"${currentFamilyTree.name}" is Empty`}
                                    description="Start building your tree by adding the first person or importing data."
                                    action={
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => {
                                                    // This is a quick hack to trigger 'Add Person' modal logic
                                                    // Ideally we lift state or use a global modal store
                                                    // For now, let's just use the floating 'Add' button or implement a direct trigger
                                                    // Since EditPersonModal requires a 'person' object, we create a dummy 'new' person
                                                    setSelectedPerson({
                                                        id: 'new',
                                                        firstName: '',
                                                        lastName: '',
                ...
                <ConfirmModal
                    ...
                    title="Reset Layout"
                    message="Are you sure you want to reset all custom node positions? This action cannot be undone."
                    confirmText="Reset"
                    isDestructive={true}
                ...
                {isEditModalOpen && selectedPerson && (
                    <EditPersonModal
                        ...
                        onClose={() => {
                            ...
                            // setSelectedPerson(null); // Keep selected for quick view?
                        }}
                        onFocus={(personId, mode) => {
                            // Implement focus logic if needed, or pass through
                            console.log('Focus:', personId, mode);
                        }}
                    />
                )}
